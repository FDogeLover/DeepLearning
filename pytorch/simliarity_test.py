...
from QuickDrwa_CNN import QuickDrawCNN  # 确保模型定义在这个模块中
...
category_paths = [f"similarity_row/full_numpy_bitmap_{name}.npy" for name in category_names]
...
def train():
    ...
    # 保存模型
    torch.save(model.state_dict(), "model/similarity_model_weights.pth")
    torch.save(model, "model/similarity_model.pkl")
